...
- **【新增】生成道具设定图**：根据道具详细信息生成道具外观设定图
- **【新增】单独生成/重新生成设定图**：支持为现有道具生成或重新生成设定图
...
在以下情况下可以为道具生成设定图：
- **新建道具**：首次创建道具时，若有外观描述
- **外观修改**：修改了道具的外观描述（外观、形态等字段）时
- **手动触发**：用户主动要求生成或重新生成

**注意**：如果没有外观描述信息，则不生成设定图。

### 5.2 道具设定图规格

**图片尺寸**：根据道具大小比例自行裁定（推荐 1024x1024 或 768x768）

**区域划分**（根据道具类型调整）：
- **小型道具**（丹药、符箓、信物等）：整体展示 + 特写
- **中型道具**（法宝、兵器、防具等）：多角度展示
- **大型道具**（灵宠、阵法等）：整体外观 + 细节特写

### 5.3 提取外观信息

从道具属性中提取外观相关信息：
- `外观` 或 `description`：道具外观描述
- `形态` 或 `form`：道具形态描述
- `特效` 或 `effect`：道具特效描述
- `颜色` 或 `color`：道具颜色描述
...
```
/webnovel-item <道具名称> --generate-image
/webnovel-item 筑基丹 --generate-image
```
...
```bash
# 查询道具信息
ls "${PROJECT_ROOT}/设定集/道具库/"*/"${道具名}.md"
```
...
- 如果没有外观描述字段，提示用户："该道具没有外观描述信息，无法生成设定图"
- 如果有外观描述，继续执行

**G1.4: 检查是否已有设定图**

```bash
ls "${PROJECT_ROOT}/设定集/道具库/${类型}/${道具名}/道具设定.png"
```

- 如果已存在，提示用户可使用 `--regenerate-image` 重新生成
- 如果不存在，执行生成

**G1.5: 生成道具设定图**

使用道具的外观描述构建提示词，调用图片生成服务生成图片。

```bash
# 使用图片生成辅助模块
python "${SCRIPTS_DIR}/image_generator.py" \
    "一颗金色筑基丹，散发着柔和的光芒" \
    "${PROJECT_ROOT}/设定集/道具库/丹药/筑基丹/道具设定.png" \
    "512x512"
```
...
```
/webnovel-item <道具名称> --regenerate-image
/webnovel-item 筑基丹 --regenerate-image
```
