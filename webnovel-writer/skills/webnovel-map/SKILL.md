...
- **【新增】生成地图风格设定图**：根据地图详细信息生成风格设定图
- **【新增】单独生成/重新生成设定图**：支持为现有地图生成或重新生成风格设定图
- 支持以下地图类型：
...
- [ ] Step 6: 更新索引和状态
- [ ] Step 7: 生成地图风格设定图（可选，若有风格设定）
- [ ] Step G1: 单独生成地图风格设定图
- [ ] Step G2: 重新生成地图风格设定图
- [ ] Step G3: 批量生成所有地图的风格设定图
```
...
- **--generate-image 参数**：生成地图风格设定图
- **--regenerate-image 参数**：重新生成地图风格设定图
- **--generate-all 参数**：批量生成所有地图的风格设定图
...
#### 生成风格设定图
```
/webnovel-map 青阳城 --generate-image
/webnovel-map 青阳城 --regenerate-image
/webnovel-map --generate-all
/webnovel-map --generate-all --type 城镇地图
```
...
| `/webnovel-map <名称> --generate-image` | 为指定地图生成风格设定图 |
| `/webnovel-map <名称> --regenerate-image` | 重新生成指定地图的风格设定图 |
| `/webnovel-map --generate-all` | 批量生成所有地图的风格设定图 |
...
在以下情况下可以为地图生成风格设定图：
- **新建地图**：首次创建地图时，若有风格设定
- **风格修改**：修改了地图的风格描述（风格、建筑特点等字段）时
- **手动触发**：用户主动要求生成或重新生成

**注意**：如果没有风格设定信息（如建筑风格、整体氛围等），则不生成设定图。

### 7.2 风格设定图规格

**图片尺寸**：1024 x 1024 像素（可根据地图类型调整）
...
```
/webnovel-map <地图名称> --generate-image
/webnovel-map 青阳城 --generate-image
```
...
```bash
# 查询地图信息
ls "${PROJECT_ROOT}/设定集/地图库/"*/"${地图名称}.md"
```
...
- 如果没有风格相关字段，提示用户："该地图没有风格设定信息，无法生成风格设定图"
- 如果有风格设定，继续执行

**G1.4: 检查是否已有设定图**

```bash
ls "${PROJECT_ROOT}/设定集/地图库/${类型}/${地图名称}/风格设定.png"
```
...
使用地图的风格描述构建提示词，调用图片生成服务生成图片。

```bash
# 使用图片生成辅助模块
python "${SCRIPTS_DIR}/image_generator.py" \
    "古风城镇地图，青砖黑瓦，街道繁华" \
    "${PROJECT_ROOT}/设定集/地图库/城镇地图/青阳城/风格设定.png" \
    "1024x1024"
```
...
```
/webnovel-map <地图名称> --regenerate-image
/webnovel-map 青阳城 --regenerate-image
```
...
## 原因
该地图没有风格设定信息，无法生成风格设定图。

## 建议
如需生成风格设定图，请在地图文件中添加以下字段：
- style: 建筑风格
- 氛围: 整体氛围
- 色调: 色调描述
```
