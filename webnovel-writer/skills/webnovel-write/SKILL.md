...
- `references/writing-standards.md`
  - 用途：写作基本规范（换段规范、引号使用规范、连接词/时态词/怀疑词规范）。
  - 触发：Step 2A 写作必读，Step 4 润色必读。
  - 重点章节：第 10 章（连接词与转折词）、第 11 章（时态表达词）、第 12 章（内心独白与怀疑词）、第 13 章（综合运用示例）
...
2. 对每个出场角色：
   - 从数据库中获取角色信息（角色库数据存储在index.db的entities表中）
   - 更新"出场记录"部分：
     - 首次出场章节：若角色首次出场，更新此字段
     - 最后出场章节：更新为当前章节
     - 出场章节列表：添加当前章节
     - 本章出场摘要：提取本章中角色出场的内容摘要
3. 如果角色信息不存在，跳过该角色的更新
