...
1. 计算需要扩写的字数 = 目标字数 × (1 - 当前字数/目标字数)
2. **确定扩写范围**：开头（章节前 200-400 字）不扩写，结尾（章节后 100-200 字）不扩写
3. 定位可扩写段落（仅限中间部分）
4. 执行扩写
5. 检查一致性
6. 若有新细节，补充到设定文件
...
2. 允许平缓收尾，但不能机械截断。
3. **【新增】连接词/时态词检查**：
   - 每 100 字至少使用 2-3 个连接词（如：就、但、虽然、然后、接着、紧接着）
   - 时态词表达时间推进（了、还、又、已经、再次）
   - 内心独白中使用怀疑词（难道、恐怕、莫不是、难道说）
4. **【新增】内心独白怀疑词检查**：
   - 分析推理时使用怀疑词增强思考感
   - 犹豫不决时配合问号使用
   - 震惊恍然时使用”原来”、”竟然”、”难怪”
5. **【硬性要求】避免连续重复**：
   - 连续出现的精确定义：
     - 跨段落重复：连续2个以上段落中各自使用了相同的词
     - 同段落内连续重复：同一段落连续2个以上句子使用了相同的词
     - 多次重复：200字内同一词出现3次及以上
   - 连续出现2次起开始警惕，3次及以上必须修改
   - 允许间隔出现（如第1句和第5句用"于是"，中间用了其他词）
   - 如需强调而连续使用（如情感爆发），每章不超过 1-2 次
...
- 毒点风险: pass/fail
- 写作规范检查:
  - 连接词使用: pass/fail（每100字2-3个）
  - 时态词使用: pass/fail（了、还、又、已经等）
  - 怀疑词使用: pass/fail（难道、恐怕等）
  - 避免连续重复:
    - 连接词: pass/fail（检查连续2段或2句以上是否用同一词）
    - 时态词: pass/fail（检查连续2句以上是否用同一词）
    - 怀疑词: pass/fail（检查内心独白中是否连续使用同一词）
- 偏离记录:
  - {位置}: {原因}
```
